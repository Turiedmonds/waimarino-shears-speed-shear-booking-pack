...
function showStep(step) {
  ...
  els.stepButtons.forEach(b => b.classList.toggle('active', b.dataset.step === target));
...
function makeEventData(existing = null) {
  return existing || { cleanShear: false, cleanShearTimeLimit: '', prizePlacings: 3, rounds: defaultRounds() };
}

function syncEventSections() {
  const names = selectedGrades();
  const current = collectEventsFromDom();
  Object.keys(state.competitionSetup.events).forEach(name => { if (!current[name]) current[name] = state.competitionSetup.events[name]; });
...
function renderEventSection(name, data) {
  ...
  fragment.querySelector('.add-round-btn').addEventListener('click', () => {
    addRoundRow(roundList, { name: 'Heats', sheepPerShearer: 1, qualifiers: null });
    renumberRounds(roundList);
  });
  (Array.isArray(data.rounds) && data.rounds.length ? data.rounds : defaultRounds()).forEach(round => addRoundRow(roundList, round));
  els.eventConfigs.appendChild(fragment);
  renumberRounds(section.querySelector('.round-list'));
}

function addRoundRow(roundList, round) {
  ...
  const standard = ['Heats', 'Quarter-final', 'Semi-final', 'Final'];
  if (standard.includes(round.name)) select.value = round.name;
  else { select.value = 'custom'; customInput.value = round.name || ''; customInput.classList.remove('hidden'); }
  ...
  function updateRoundFields() {
    ...
    const isFinal = select.value === 'Final';
  ...
  fragment.querySelector('.remove-round-btn').addEventListener('click', () => { row.remove(); renumberRounds(roundList); });
  updateRoundFields();
  roundList.appendChild(fragment);
}
function renumberRounds(roundList) {
  [...roundList.querySelectorAll('.round-row')].forEach((row, i) => row.querySelector('.round-number').textContent = i + 1);
}
...
function collectEventsFromDom() {
  ...
  els.eventConfigs.querySelectorAll('.event-card').forEach(section => {
    ...
    const rounds = [...section.querySelectorAll('.round-row')].map(row => {
      ...
      const isFinal = select.value === 'Final' || /^final$/i.test(roundName);
      return {
        name: roundName,
    ...
    events[name] = { cleanShear, cleanShearTimeLimit, prizePlacings, rounds };
  });
  return events;
}
...
function validateForReview() {
  ...
  events.forEach(([eventName, event]) => {
    if (!event.rounds?.length) warnings.push(`${eventName}: no rounds have been added.`);
    ...
    event.rounds?.forEach((round, index) => {
      ...
      if (!isFinal && round.qualifiers == null && index < event.rounds.length - 1) warnings.push(`${eventName} — ${round.name || `round ${index + 1}`}: number qualifying to the next round is missing.`);
    });
  });
  if (state.competitionSetup.judging.boardJudge && !state.competitionSetup.judging.boardJudges) warnings.push('Board judge is selected but the number of board judges is missing.');
  return warnings;
}
...
function buildReview() {
  ...
  els.reviewWarnings.innerHTML = warnings.length ? `<strong>Please check these items before sending the booking:</strong><ul>${warnings.map(w => `<li>${escapeHtml(w)}</li>`).join('')}</ul>` : '';
  const eventsHtml = Object.entries(state.competitionSetup.events || {}).map(([name, event]) => {
    const rounds = (event.rounds || []).map(r => `<li><strong>${escapeHtml(r.name || 'Unnamed round')}</strong> — ${r.sheepPerShearer || '—'} sheep per shearer${r.qualifiers == null ? '' : ` • ${r.qualifiers} qualify`}</li>`).join('');
...
function saveDraft(showMessage = true) {
  syncStateFromForm(); ensureIds(); localStorage.setItem(STORAGE_KEY, JSON.stringify(buildPackage(false)));
...
function buildHumanPackHtml() {
  buildReview();
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(state.booking.competitionName || 'Speed Shear')} — Booking Pack</title><style>body{font-family:Arial,sans-serif;color:#17212b;max-width:900px;margin:30px auto;padding:0 20px;line-height:1.5}h1{border-bottom:3px solid #173f35;padding-bottom:10px}.status{padding:12px;background:#fff8df;border:1px solid #e2c65d;border-radius:8px}.review-section{border:1px solid #dbe3ea;border-radius:10px;padding:18px;margin:14px 0}.review-list{display:grid;grid-template-columns:1fr 1fr;gap:8px 18px}.review-item{border-bottom:1px solid #e5e7eb;padding:5px 0}.review-item span{display:block;font-size:11px;text-transform:uppercase;color:#64748b}.review-event{border:1px solid #dbe3ea;border-radius:8px;padding:12px;margin-top:10px}@media(max-width:650px){.review-list{grid-template-columns:1fr}}</style></head><body><h1>Waimarino Shearing — Speed Shear Hire &amp; Booking Pack</h1><p class="status"><strong>Booking status:</strong> Submitted. This booking is not confirmed until the deposit has been paid.</p>${els.reviewContent.innerHTML}<section class="review-section"><h3>Hire information summary</h3><p>Standard hire includes the two-stand speed shear stand, two Heiniger Evo shearing plants, timing system, two TVs, judging devices, catching pens, backup timing equipment, required extension leads, transport, setup, testing, pack-down and system operators.</p><p>The organiser provides the stage/trailer or suitable setup area, suitable power, lighting, PA/sound, generator if required, additional yards/pens, competition judges, sheep handlers and entry/sign-in staff.</p><p><strong>Space:</strong> approximately 4.8 m wide × 2.9 m deep × 2.45 m high, plus additional safe working space. Allow approximately four hours for setup and testing.</p></section><section class="review-section"><h3>Terms acknowledgement</h3><p>The organiser has accepted Waimarino Shearing Incorporated Hire Terms &amp; Conditions version ${escapeHtml(TERMS_VERSION)}. Accepted by ${escapeHtml(state.booking.acceptedBy || '—')} on ${escapeHtml(humanDateTime(state.booking.acceptedAt))}.</p></section></body></html>`;
}
function downloadHumanPack() {
  syncStateFromForm(); ensureIds();
...
function applyStateToForm() {
  ...
  eventNames.forEach(name => {
    ...
    if (standard && name !== 'Other') standard.checked = true;
    else if (!customName) customName = name;
  });
  if (customName) {
    els.otherGradeToggle.checked = true; els.otherGradeName.value = customName; els.otherGradeWrap.classList.remove('hidden');
  } else {
    els.otherGradeName.value = ''; els.otherGradeWrap.classList.add('hidden');
  }
  els.eventConfigs.innerHTML = '';
...
function loadPackage(pack, notify = true) {
  ...
  state.competitionSetup = { events: pack.competitionSetup?.events || {}, judging: { ...state.competitionSetup.judging, ...(pack.competitionSetup?.judging || {}) } };
  applyStateToForm();
...
function loadDraft() {
  ...
  if (!raw) return;
  try { loadPackage(JSON.parse(raw), false); } catch (error) { console.warn('Could not load saved draft:', error); }
}
function showToast(message) {
  document.querySelector('.toast')?.remove();
  const toast = document.createElement('div'); toast.className = 'toast'; toast.textContent = message; document.body.appendChild(toast); setTimeout(() => toast.remove(), 2400);
}

function bindEvents() {
  document.querySelectorAll('.next-step').forEach(button => button.addEventListener('click', () => { syncStateFromForm(); showStep(button.dataset.next); }));
  document.querySelectorAll('.prev-step').forEach(button => button.addEventListener('click', () => showStep(button.dataset.prev)));
  els.stepButtons.forEach(button => button.addEventListener('click', () => { syncStateFromForm(); showStep(button.dataset.step); }));
  ...
  document.getElementById('printBookingPackBtn').addEventListener('click', () => { buildReview(); window.print(); });
  els.bookingFileInput.addEventListener('change', async () => {
    const file = els.bookingFileInput.files?.[0]; if (!file) return;
    try { loadPackage(JSON.parse(await file.text())); showStep(2); }
    catch (error) { window.alert(`Could not open this booking file. ${error.message}`); }
    finally { els.bookingFileInput.value = ''; }
  });
  window.addEventListener('beforeunload', () => { try { saveDraft(false); } catch (_) {} });
}
